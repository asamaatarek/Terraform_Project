                ...
                script {
                    if (BASTION_IP_AZ1 != null && BASTION_IP_AZ1 != '') {
                        withCredentials([sshUserPrivateKey(credentialsId: 'tera-pem', keyFileVariable: 'PEM_FILE', usernameVariable: 'ubuntu')]) {
                            sh """#!/bin/bash
                                chmod 400 $PEM_FILE
                                scp -o StrictHostKeyChecking=no -i $PEM_FILE -r ansible/roles/docker_nginx/tests/inventory ansible/roles/docker_nginx/tests/test.yml ansible/deploy_nginx.yml ubuntu@${BASTION_IP_AZ1}:/home/ubuntu/
